...
public class Pain001V00103CH02Service implements Pain001Service {
	...
	private String getXMLStringFromDocument(final Document document) {
		...
		try {
			...
			throw new Iso20022RuntimeException("Unerwarteter Fehler beim generieren des Zahlungsfile: Failed to "
					+ "marshal Document", e);
		}
	...
	private JAXBElement<Document> getElementToMarshall(Document elemToMarshall) {
		return new JAXBElement<>(new QName(SCHEMA_LOCATION, elemToMarshall.getClass().getSimpleName()), Document
				.class, elemToMarshall);
	}
	...
	private Document createPain001Document(Pain001DTO pain001DTO) {
		...
		String debtor_name = pain001DTO.getSchuldnerName();
		String debtor_bic = pain001DTO.getSchuldnerBIC();
		String debtor_iban = pain001DTO.getSchuldnerIBAN();
		String debtor_iban_gebuehren = pain001DTO.getSchuldnerIBAN_gebuehren();
		...
		if (debtor_iban == null) {
			throw new Iso20022RuntimeException("Schuldner Konto IBAN Nummer ist leer: debtor_iban muss angegeben werden");
		}
		...
		PaymentInstructionInformation3CH paymentInstructionInformation3CH = createPaymentInstructionInformation3CH
				(pain001DTO, objectFactory, debtor_name, debtor_iban, debtor_bic, debtor_iban_gebuehren);
		...
		for (AuszahlungDTO auszahlungDTO : pain001DTO.getAuszahlungen()) {
			...
			document.getCstmrCdtTrfInitn().getPmtInf().get(0).getCdtTrfTxInf().add
					(createCreditTransferTransactionInformation10CH(objectFactory, transaktion, auszahlungDTO,
							pain001DTO.getAuszahlungsDatum()));
		}

		document.getCstmrCdtTrfInitn().setGrpHdr(createGroupHeader(pain001DTO, objectFactory, transaktion, ctrlSum,
				debtor_name));
	 ...
	 * </pre>
	 */
	private CreditTransferTransactionInformation10CH createCreditTransferTransactionInformation10CH(ObjectFactory
			objectFactory, int transaktion, AuszahlungDTO auszahlungDTO, LocalDate date) {
		...
		cTTI10CH.getCdtrAgt().getFinInstnId().setClrSysMmbId(objectFactory.createClearingSystemMemberIdentification2
				());
		cTTI10CH.getCdtrAgt().getFinInstnId().getClrSysMmbId().setClrSysId(objectFactory
		...
		String endToEndId = transaktion + "/" +
				date.getMonthValue() + "/" +
				auszahlungDTO.getZahlungsempfaegerName();
		...
		cTTI10CH.getCdtrAgt().getFinInstnId().getClrSysMmbId().setMmbId(auszahlungDTO
				.getZahlungsempfaegerBankClearingNumber());
		...
		cTTI10CH.getCdtrAcct().getId().setIBAN(auszahlungDTO.getZahlungsempfaegerIBAN().replaceAll(" ", "")); // 2.80

		// 1.1.1	ETAB 503.2: EZAG für PC- / IBAN-Auszahlung aufbereiten, 2. Teil
		// Strukturierte Daten
		cTTI10CH.setCdtr(objectFactory.createPartyIdentification32CHName());
		cTTI10CH.getCdtr().setNm(normalize(auszahlungDTO.getZahlungsempfaegerName())); // 2.79
		cTTI10CH.getCdtr().setPstlAdr(objectFactory.createPostalAddress6CH());
		...
		if (auszahlungDTO.getZahlungText() == null) {
			...
			cTTI10CH.getRmtInf().setUstrd(normalize(auszahlungDTO.getZahlungsempfaegerName() + ", Monat " + monat + ""
					+ "." + date.getYear()));    // 2.99
		} else {
	...
	private PaymentInstructionInformation3CH createPaymentInstructionInformation3CH(
		...
		paymentInstructionInformation3CH.setReqdExctnDt(getXmlGregorianCalendar(pain001DTO.getAuszahlungsDatum()
				.atStartOfDay()));
	 ...
	 * </pre>
	 */
	private GroupHeader32CH createGroupHeader(Pain001DTO pain001DTO, ObjectFactory objectFactory, int transaktion,
			BigDecimal ctrlSum, String debtor_name) {
		// GroupHeader
		// struktur
