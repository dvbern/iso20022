...
public class Camt054V00104Service extends AbstractCamtService implements Camt054Service {
	...
	public static final String PATH_CAMT_054_001_04_XSD =
			"ch/dvbern/oss/lib/iso20022/camt054/v00104/camt.054.001.04.xsd";
	 ...
	 */
	@Override
	public BankToCustomerDebitCreditNotificationV04 getNotificationFromXml(byte[] xmlAsBytes) {
	...
	private BankToCustomerDebitCreditNotificationV04 unmarshallNotificationFromXml(byte[] xmlAsBytes) {
		...
		BankToCustomerDebitCreditNotificationV04 notification = new BankToCustomerDebitCreditNotificationV04();
		...
		try (ByteArrayInputStream xmlAsStream = new ByteArrayInputStream(xmlAsBytes)) {
			...
			notification = doc.getValue().getBkToCstmrDbtCdtNtfctn();
	...
	public DocumentDTO getDocumentWithBookedEsrPaymentsFromXml(byte[] xmlAsBytes) throws Iso20022RuntimeException {
		...
		if (validateXmlWithXsd(xmlAsBytes, PATH_CAMT_054_001_04_XSD)) {
			documentDTO.setCamtTypeVersion(CamtTypeVersion.CAMT054V00104);
		} else {
			throw new Iso20022RuntimeException("SAX Exception while validating XML File");
		}
		...
		for (AccountNotification7 accountNotification7 : notification7List) {
			BAccountStatementDTO bAccountStatementDTO = getbAccountStatementDTO(accountNotification7);
	...
	private AGroupHeaderDTO getaGroupHeaderDTO(BankToCustomerDebitCreditNotificationV04 notification) {
	...
	private BAccountStatementDTO getbAccountStatementDTO(AccountNotification7 accountNotification7) {
		BAccountStatementDTO bAccountStatementDTO = new BAccountStatementDTO();
		bAccountStatementDTO.setIdentification(accountNotification7.getId());
		bAccountStatementDTO.setAccountIdentificationIBAN(accountNotification7.getAcct().getId().getIBAN());
		XMLGregorianCalendar creationDateTime = accountNotification7.getCreDtTm();
		bAccountStatementDTO.setCreationDateTime(creationDateTime.toGregorianCalendar().toZonedDateTime()
				.toLocalDateTime());

		// 'Statement Entry' (C-Level) (0..n)
		final List<ReportEntry4> entries = accountNotification7.getNtry();
